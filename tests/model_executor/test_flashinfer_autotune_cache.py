...
def test_deepseek_v4_sparse_mla_backend_detection_is_dsv4_scoped() -> None:
    assert kernel_warmup._has_deepseek_v4_sparse_mla_backend(
        make_runner(["V4_FLASHMLA_SPARSE"])
    )
    assert kernel_warmup._has_deepseek_v4_sparse_mla_backend(
        make_runner(["V4_FLASHINFER_MLA_SPARSE"])
    )
    assert not kernel_warmup._has_deepseek_v4_sparse_mla_backend(
        make_runner(["FLASHINFER_MLA_SPARSE"])
    )


def test_flashinfer_sparse_mla_autotune_context_selects_context_attr() -> None:
    dsv3_2_context = kernel_warmup._flashinfer_sparse_mla_decode_autotune_context(
        make_runner(["FLASHINFER_MLA_SPARSE"]),
        kernel_warmup._FLASHINFER_MLA_SPARSE_BACKENDS,
    )
    assert dsv3_2_context is not None
    assert dsv3_2_context[0] == "sparse_mla_sm120_decode_dsv3_2_autotune"

    dsv4_context = kernel_warmup._flashinfer_sparse_mla_decode_autotune_context(
        make_runner(["V4_FLASHINFER_MLA_SPARSE"]),
        kernel_warmup._DEEPSEEK_V4_FLASHINFER_MLA_SPARSE_BACKENDS,
    )
    assert dsv4_context is not None
    assert dsv4_context[0] == "sparse_mla_sm120_decode_dsv4_autotune"
...
def test_sparse_mla_autotune_broadcast_writes_nonlocal_rank_cache(
    ...
    monkeypatch.setattr(
        kernel_warmup, "_resolve_flashinfer_autotune_file", lambda runner: cache_path
    )
    ...
    assert kernel_warmup._deepseek_v4_sparse_mla_decode_autotune(worker, 16) is True
...
def test_sparse_mla_autotune_uses_dsv3_2_context_on_leader(
    ...
    assert kernel_warmup._flashinfer_sparse_mla_decode_autotune(worker, 16) is True
...
def test_sparse_mla_autotune_v2_returns_false_when_mixed_warmup_skips(
    ...
    assert kernel_warmup._deepseek_v4_sparse_mla_decode_autotune(worker, 2) is False
...
def test_flashinfer_sparse_mla_warmup_skips_non_sm120_without_dummy(
    ...
    kernel_warmup._flashinfer_sparse_mla_decode_autotune_warmup(worker)
